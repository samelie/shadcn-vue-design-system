...
<template>
  <div class="space-y-8">
    <section>
      <h2 class="text-2xl font-semibold mb-4">Single Date Calendar</h2>
      <div class="flex gap-8">
        <div>
          <Calendar v-model="singleDate" />
        </div>
        <div>
          <p class="text-sm text-muted-foreground mb-2">Selected Date:</p>
          <p class="font-mono">{{ formatDate(singleDate) }}</p>
        </div>
      </div>
    </section>

    <section>
      <h2 class="text-2xl font-semibold mb-4">Date Range Calendar</h2>
      <div class="flex gap-8">
        <div>
          <RangeCalendar v-model="dateRange" />
        </div>
        <div>
          <p class="text-sm text-muted-foreground mb-2">Selected Range:</p>
          <p class="font-mono">{{ formatDateRange(dateRange) }}</p>
        </div>
      </div>
    </section>

    <section>
      <h2 class="text-2xl font-semibold mb-4">Calendar in Popover (Date Picker)</h2>
      <Popover>
